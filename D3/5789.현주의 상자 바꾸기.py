...
for tc in range(int(input())):
    N, Q = map(int, input().split())        # N: 박스 수 Q: 넘버링 행동 횟수
    box = [0]*N

    for i in range(1,Q+1):                  # i: Q번 동안 넘버링 할 숫자
        L, R = map(int, input().split())    # 숫자 i를 새길 상자의 범위
        for j in range(L, R+1):             # L ~ R까지
            box[j-1] = i                    # 인덱스 맞추기 위해 j-1 숫자 i
