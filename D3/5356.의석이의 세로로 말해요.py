...
for tc in range(int(input())):
    ...
    for i in word:
        if len(i) > max:
            max = len(i)            # 가장 긴 단어의 길이를 구한다
    for i in word:
        if len(i) < max:            # 가장 긴 단어보다 짧은 단어라면
            while len(i) <= max:    # 부족한 길이만큼 정수 0을 붙여준다
                i += [0]
                
    print(f'#{tc+1}', end=' ')
    for i in range(max):            
        for j in range(5):          # 세로로 읽으며
            if word[j][i] == 0:     # 정수 0이라면 pass한다 입력받은 0은 str이기 때문에 괜찮다
                pass
            else:
                print(word[j][i], end='')
    print()
# b = list(map(list, zip(*word)))
# print(f'#{tc+1}', end=' ')
# for i in b:
#     print(''.join(i), end='')
# print()
