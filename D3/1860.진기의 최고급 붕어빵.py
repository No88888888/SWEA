...
for tc in range(int(input())):
    N, M, K = map(int, input().split()) # N:손님 수 M:M초동안 K:K개만큼 붕어빵
    A = [0 for _ in range(N*M+1)]           
    for i in range(1, len(A)):          # 해당 초마다 만들어 낼 붕어빵 리스트 작성    
        if i%M == 0:
            A[i] = K

    time = list(map(int, input().split()))  # 해당 초에 오는 손님 리스트
    T = [0]*(max(time)+1)
    for i in time:
        T[i] = i
        
    boong, son = 0, 0               # 붕어빵 수, 손님 수
    t = 0                           # 시간 초
    result = 'Possible'
    while t <= N*M:
        
        if A[t] != 0:               # 붕어빵 만들어내는 시간이면
            boong += K              # K개만큼 추가
        if t in time:               # 손님이 오는 시간이면
            son += 1                # 손님 수 추가
        if boong - son < 0:         # 그대 붕어빵 수 - 손님 수가 음수이면
