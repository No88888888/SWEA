...
for tc in range(int(input())):
    N, M = map(int, input().split())        # N,M 행열
    flag = [list(input()) for _ in range(N)]# 깃발 초기값
    result = []
    for a in range(1, N-1):                 # 인덱스 설정이 관건
        for b in range(a+1,N):
            count = 0
            for i in range(a):              # 흰색이 칠해질 구간
                count += flag[i].count('R') + flag[i].count('B')
            for j in range(a, b):           # 파란색이 칠해질 구간
                count += flag[j].count('W') +flag[j].count('R')
            for k in range(b, N):           # 빨간색이 칠해질 구간
                count += flag[k].count('W') +flag[k].count('B')
            result.append(count)            #가능한 경우의 수 전수 조사
    print(f'#{tc+1} {min(result)}')         # 그 중 최솟값 도출
