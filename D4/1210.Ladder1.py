...
#                     ni = y + di[i]
#                     nj = x + dj[i]
#                     if 0<=ni<10 and 0<=nj<10 and ladder[ni][nj]:
#                         y = ni
#                         x = nj
#
#                     elif ladder[y][x] == 2:
#                         print(f'#{tc} {j}')
#                     elif y == 9 and ladder[y][x] == 1:
#                         y += 1
#                         break
                    
# 지나온 길로 다시 돌아가지 않게 구현 실패
...
for _ in range(1, 11):
    ...
    for i in range(100):
        if ladder[99][i] == 2:          # 거꾸로 밑에서 2인곳을 찾아 역으로 올라옴
            x = 99
            y = i                       # 값이 인 곳의 y좌표
        
    while x != 0:                       # 출발지의 x좌표가 0이기 때문
        for j in range(3):
            nx = x + dx[j]              # 좌 우 상 순서로 탐색
            ny = y + dy[j]
